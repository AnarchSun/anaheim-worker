# PATH: hyper_safe_hot_reload_worker.py
...
from ts_worker import auto_ts_fix_cycle, handle_ts_error  # assure-toi que ts_worker.py est correct
...
MAX_COMMITS_PER_BRANCH = 1      # max commits allowed per interval
COMMIT_INTERVAL = 300            # 5 minutes
BUFFER_LIMIT = 100               # max patches in buffer before commit
...
def main_hot_reload(num_threads: int = 4):
    ...
    try:
        ...
        for t in threads:
            t.join(timeout=2)
        log("✅ Hyper safe hot reload worker stopped cleanly.")
