...
from ts_worker import auto_ts_fix_cycle, handle_ts_error  # <-- intégration live TS
    ...
    while not shutdown_event.is_set():
        try:
            ...
            if isinstance(task, dict):
                ...
                # In live mode, LLM task → auto_ts_fix_cycle
                try:
                    patches, _ = auto_ts_fix_cycle(task)
                    for patch in patches:
                        # appliquer patch directement
...
# -----------------------
# Auto TS fix cycle safe (buffers commits)
# -----------------------
def auto_ts_fix_cycle_safe(repo_obj: Optional[Repo] = None,
                           last_commit_time: float = 0.0,
                           target_branch: str = "Orion",
                           action_buffer: Optional[List[dict]] = None) -> tuple[List[dict], float]:
    applied_actions, last_commit_time = auto_ts_fix_cycle(repo_obj=repo_obj, target_branch=target_branch, last_commit_time=last_commit_time)
    if action_buffer is not None and applied_actions:
        action_buffer.extend(applied_actions)
        now_ts = time.time()
        if repo_obj and (now_ts - last_commit_time >= 300):  # commit toutes les 5 min max
            try:
